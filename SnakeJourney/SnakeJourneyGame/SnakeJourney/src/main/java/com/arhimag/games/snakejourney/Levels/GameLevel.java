...
import com.arhimag.games.snakejourney.Achievements.GetLength40;
import com.arhimag.games.snakejourney.Achievements.UseTeleport;
...
public abstract class GameLevel
{
    ...
    boolean messageScreen = false;

    GameMap messageScreenMap = null;
...
//		if ( (tempMap[x1][y1] == 'T') && (tempMap[x2][y2] == 'T'))
						...
						else
						{
							for( int i = 0; i < food.length; i++)
							{
								if (( nextFieldX == food[i].x ) && ( nextFieldY == food[i].y ))
								{
                                    ...
                                    if (snakeid != playerSnake && snakes[snakeid].parts.size() > snakes[snakeid].finishSize)
                                    {
                                        if (Settings.isSoundEnabled())
                                            Assets.opponentFinish.play(1);
                                    }
                                    else if (snakeid == playerSnake && snakes[snakeid].parts.size() > snakes[snakeid].finishSize  )
                                    {
                                        if (Settings.isSoundEnabled())
                                            Assets.finish.play(1);
                                    }
									eat = true;
    ...
    public boolean getMessageScreen() { return messageScreen;};
    public void setMessageScreen( boolean newValue ) { messageScreen = newValue; };

    public GameMap getMessageScreenMap() { return messageScreenMap;};
    public void setMessageScreenMap( GameMap newValue ) { messageScreenMap = newValue; };
