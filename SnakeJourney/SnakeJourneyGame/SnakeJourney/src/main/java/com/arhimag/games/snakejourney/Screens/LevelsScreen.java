...
public class LevelsScreen extends Screen
{
	...
	private void goToLevelTL()
	{
		if( ((LevelLevelsList)this.levelDrawer.getLevel()).getTopLeft() != null )
			game.setScreen(new GameScreen(game,LevelSequence.createLevelByMap(((LevelLevelsList)this.levelDrawer.getLevel()).getTopLeft())));
	}
	
	private void goToLevelTR()
	{
		if( ((LevelLevelsList)this.levelDrawer.getLevel()).getTopRight() != null )
			game.setScreen(new GameScreen(game,LevelSequence.createLevelByMap(((LevelLevelsList)this.levelDrawer.getLevel()).getTopRight())));
	}
	
	private void goToLevelBL()
	{
		if( ((LevelLevelsList)this.levelDrawer.getLevel()).getBottomLeft() != null )
			game.setScreen(new GameScreen(game,LevelSequence.createLevelByMap(((LevelLevelsList)this.levelDrawer.getLevel()).getBottomLeft())));
	}
	
	private void goToLevelBR()
	{
		if( ((LevelLevelsList)this.levelDrawer.getLevel()).getBottomRight() != null )
			game.setScreen(new GameScreen(game,LevelSequence.createLevelByMap(((LevelLevelsList)this.levelDrawer.getLevel()).getBottomRight())));
	}
	...
	public void update (float deltaTime)
	{
		...
		for(int i = 0; i < len; i++)
		{
			...
			if(event.type == TouchEvent.TOUCH_UP)
			{
				...
				{
                    if( levelDrawer.inBounds(event, LevelsListMap.getNextBounds()) )
                    {
                        if( Settings.isSoundEnabled())
                            Assets.eat.play(1);
                        ((LevelLevelsList)this.levelDrawer.getLevel()).incListNumber();
                    }

                    if( levelDrawer.inBounds(event, LevelsListMap.getPreviousBounds()))
                    {
                        if( Settings.isSoundEnabled())
                            Assets.eat.play(1);
                        ((LevelLevelsList)this.levelDrawer.getLevel()).decListNumber();
                    }
