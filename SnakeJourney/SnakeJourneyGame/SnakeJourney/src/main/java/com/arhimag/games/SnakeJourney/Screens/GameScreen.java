...
import com.arhimag.games.SnakeJourney.Social.VKontakteAuth;
		...
		if( this.levelDrawer.getLevel().getEggsWindow())
		{
			...
			for(int i = 0; i < len; i++)
			{
				...
				if(event.type == TouchEvent.TOUCH_UP)
				{
					...
					{
						if( LevelSequence.getLevelNum(levelDrawer.getLevel()) < LevelSequence.getLevelsCount() - 2)
						{
...
//                        AndroidGame.faceBookActivityLink.runOnUiThread(FacebookRunner.fbRunner);
		...
		if( this.levelDrawer.getLevel().nextLevel() )
		{
            ...
            else if( LevelSequence.getLevelNum(levelDrawer.getLevel()) < LevelSequence.getLevelsCount() - 2)
			{
				switch(Settings.getControl())
				{
					case 0:
